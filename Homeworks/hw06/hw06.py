...
def balanced(m):
    ...
    if is_weight(m):
        return True
    return torque(left_side(m)) == torque(right_side(m)) and balanced(end(left_side(m))) and balanced(end(right_side(m)))
...
def with_totals(m):
    ...
    [None, None]
    """
    return tree(total_weight(m), [with_totals(side) for side in sides(m)])
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, pas):
        ...
        if pas == password:
            ...
            incorrect_passwords.append(pas)
            count += 1
...
def make_joint(withdraw, old_password, new_password):
    ...
    "Your account is locked. Attempts: ['my', 'secret', 'password']"
    """
    if str(withdraw(0, old_password)) == "Incorrect password":
        return "Incorrect password"
    def withdraw_joint(amount, pas):
        if pas == new_password or pas == old_password or str(withdraw(0, pas)) != "Incorrect password":
            return withdraw(amount, old_password)
        else:
            return "Incorrect password"
    if str(withdraw(0, old_password))[0] == 'Y':
