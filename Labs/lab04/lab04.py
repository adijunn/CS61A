...
def closer_city(lat, lon, city1, city2):
    ...
    'Bucharest'
    """
    city = make_city("temp_city", lat, lon)
...
def replace_elem(lst, index, elem):
    ...
    "*** YOUR CODE HERE ***"
    return [lst[e] for e in range(index)] + [elem] + [lst[e] for e in range(index+1, len(lst))]
...
def put_piece(board, max_rows, column, player):
    ...
    for r in range(max_rows):
        if get_piece(board, max_rows-r-1, column) == '-':
            new_row = replace_elem(board[max_rows-r-1], column, player)
            new_board = replace_elem(board, max_rows-r-1, new_row)
            return (max_rows-r-1, new_board)
