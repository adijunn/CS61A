...
def find_centroid(cluster):
    ...
    # BEGIN Question 5
    location_sequence = [restaurant_location(restaurant) for restaurant in cluster]
    latitude_sequence = [lat for lat, long in location_sequence]
    longitude_sequence = [long for lat, long in location_sequence]
    return [mean(latitude_sequence), mean(longitude_sequence)]
    # END Question 5
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    set = [find_predictor(user, reviewed, feature_fn) for feature_fn in feature_fns]
    max_r_squared = max([find_predictor(user, reviewed, feature_fn)[1] for feature_fn in feature_fns])
    for x, y in set:
        if y == max_r_squared:
            return x
    # END Question 8
