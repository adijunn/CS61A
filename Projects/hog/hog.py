...
def roll_dice(num_rolls, dice=six_sided):
    ...
    for curr_roll in range(1, num_rolls+1):
        result = dice()
        if result == 1:
            one_was_rolled = True
            total = 1
        if not one_was_rolled:
            total += result
    return total
    # END PROBLEM 1
...
def is_swap(score0, score1):
    ...
    # BEGIN PROBLEM 4
    bigger_score = max(score0, score1)
    smaller_score = min(score0, score1)

    if bigger_score in (0, 1) or smaller_score in (0, 1):
        return False

    if bigger_score % smaller_score == 0:
        return True
    else:
        return False
    # END PROBLEM 4
...
#def hasTimeTrot(player, n, k):
    #def f(previous):
        #return n % k == 8 and (not previous)
    #return f
...
def make_averaged(fn, num_samples=1000):
    ...
    def average(*args):
        ...
        for i in range(1, num_samples+1):
            summed_values += fn(*args)
        result = summed_values / num_samples
        return result
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    """
    # BEGIN PROBLEM 11
    theoretical_score = score + free_bacon(opponent_score)
    if score < opponent_score and is_swap(theoretical_score, opponent_score):
        return 0
