...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        #return scheme_apply(procedure, procedure.eval_call(rest, env),  env)
        # END PROBLEM 5
...
class Frame:
    ...
    def lookup(self, symbol):
        ...
        #elif value == nil and self.parent is not None:
            #return self.parent.lookup(symbol)
        # END PROBLEM 3
    ...
    def make_child_frame(self, formals, vals):
        ...
        length = len(formals)
        if length != len(vals):
            raise SchemeError("number of parameters does not match number of arguments")
        python_formals = []
        python_vals = []
        ...
        for i in range(length):
            child.define(python_formals[i], python_vals[i])
        # END PROBLEM 11
        return child
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        mapped = operands.map(lambda x: scheme_eval(x, env))
...
class MacroProcedure(LambdaProcedure):
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN Problem 21
        return operands.map(lambda x: complete_eval(x))
        # END Problem 21
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        #new_expression = Pair(target.second, expressions.second)
...
def make_let_frame(bindings, env):
    ...
    body = nil
    while bindings != nil:
        check_form(bindings.first, 2, 2.5)
        formals = Pair(bindings.first.first, formals)
        check_formals(formals)
        body = Pair(scheme_eval(bindings.first.second.first, env), body)
        bindings = bindings.second
    return env.make_child_frame(formals, body)
    # END PROBLEM 15
...
def do_define_macro(expressions, env):
    ...
    #if not isinstance expressions.first.second:
    check_form(expressions, 2)
    formals = expressions.first
    check_formals(formals)
...
def do_mu_form(expressions, env):
    ...
    check_formals(formals)
    # BEGIN PROBLEM 16
    procedure = MuProcedure(formals, expressions.second)
    return procedure
    # END PROBLEM 16
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            result = scheme_apply(proc, operands, env)
            # END PROBLEM 20
    return result
